...
float PID_Calcular(PID_Controller* pid, float valorActual, float objetivo, float dt) {
    ...
    // Suma de errores para el término integral
    pid->integral += pid->err * dt;

    // Límites integración (Anti-windup)
...
void Homing(void) {
    ...
    // Configuramos dirección de giro hacia los sensores (Pines del puerto E)
